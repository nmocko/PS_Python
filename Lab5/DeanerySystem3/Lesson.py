...
class Lesson:
    ...
    def earlier_day(self):
        ...
        if not TimetableWithBreaks.skipBreaks:
            ...
            if TimetableWithBreaks.can_be_transferred_to(termp, self.fullTime):
                self.term.duration -= 10
                termp.duration -= 10
            ...
            if TimetableWithoutBreaks.can_be_transferred_to(termp, self.fullTime):
                return Lesson(termp, self.__name, self.__teachername, self.__year)
            else:
                print("It's not possible")
                return self
    ...
    def later_term(self):
        print("I am here?")
        print(TimetableWithBreaks.skipBreaks)
        if not TimetableWithBreaks.skipBreaks:
            print("WTF")
            self.term.duration += 10
            ...
            termp = Term(Day(d), h, m, self.__term.duration)
            print(h, m, self.__term.duration)
            if TimetableWithoutBreaks.can_be_transferred_to(termp, self.fullTime):
                return Lesson(termp, self.__name, self.__teachername, self.__year)
            else:
                print("It's not possible")
